...
if st.session_state.records:
    ...
    if not is_admin():
        ...
        if "Date Sort" in df_preview.columns:
            df_preview = df_preview.drop(columns=["Date Sort"])

        df_preview_latest_10 = df_preview.head(10)
        ...
        # =====================================================
        # PREVIEW LATEST 10 + TOTAL COUNT + HIGHLIGHT LATEST
        # =====================================================

        st.subheader("📋 Preview - Latest 10 Records")
        ...
        st.caption(
            "Showing only the latest 10 records. The newest entry is highlighted and shown at the top."
        )
        ...
        if not df_preview_latest_10.empty:
            latest_row_index = df_preview_latest_10.index[0]


        def highlight_latest_row(row):
            if row.name == latest_row_index:
        ...
        if search_name:
            ...
            latest_10_search = filtered_df.head(10)

            if not latest_10_search.empty:

                styled_search_df = latest_10_search.style.apply(
            ...
            styled_latest_df = df_preview_latest_10.style.apply(
                highlight_latest_row,
                axis=1
            )
        ...
        if st.button("🗑️ Delete This Record", type="primary"):
            ...
            if confirm_delete:
                ...
                delete_result = delete_record_from_db(record_id)
