...
st.title("GRL File Registration")
...
def split_clients(client_text):
    """
    Splits Client(s) Particulars into Vendors and Purchasers.
    This helps when both V and P are inside the same Excel cell.
    """
...
def split_solicitor_values(value_text):
    """
    Keeps blank lines so values do not shift upward.
...
def safe_date_for_input(value):
    """
    Streamlit date_input cannot accept NaT.
    This safely converts date values.
    """
    ...
    if (
        ...
        try:
            ...
            while row_index < len(raw_df):
                ...
                # Skip rows without file reference
                ...
                # Some records may use the row below for Purchaser
                ...
                # Move row by row instead of every 2 rows
                row_index += 1

            st.session_state.records = imported_records
            ...
            st.success(
                f"✅ Imported {len(imported_records)} records successfully!"
            )
        ...
        st.info(
            "✅ Excel file already imported. You can edit records below."
        )
...
if st.session_state.records:
    ...
    # Do not reset index.
    # The original index is needed for editing the correct record.
